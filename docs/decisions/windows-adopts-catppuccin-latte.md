...
and turned both off: acrylic blends the window with whatever sits behind it,
so its result is inherently unpredictable, and Catppuccin Latte's foreground
already runs a moderate ~7:1 contrast against its background (`#4C4F69`
on `#EFF1F5`) with less margin than the dark scheme it replaced to spend
on an unpredictable blend. `windows/desired/files/terminal/settings.json`
is plain JSON and carries no comment syntax, so this reasoning is recorded
here rather than beside the setting.
