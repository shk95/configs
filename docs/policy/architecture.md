...
decision. The domain's documents are the third: they live in the Unix-like
scope directories under `docs/`, beside the other scopes' documents
...
candidates, the decision records and the invariant registry) under
`docs/policy/`, the provisional registry, work documents, current state and
reference material beside it. Inside each area a scope directory, or a file
named for a scope, owns what it holds, so a domain's decision record,
invariant, status or evidence item is that domain's change and lands with the
work it records; anything else under `docs/` is repository's. No area keeps a
hand-written list of its own documents: the list is printed from their
headers, and a hand-kept one would make every new record a repository edit.
...
absolute home paths reached `docs/reference/troubleshooting.md` and stayed for the
...
required by `docs/policy/definition-of-done/`, and the repository maintainer is its
...
`docs/policy/invariants/<scope>/`, one entry per invariant, rather than written as prose
...
`docs/policy/definition-of-done/`), or `pending` (an issue, until a check exists).
...
decision record under `docs/policy/decisions/` with `decision:`; the pointer is
...
`docs/provisional/<scope>/` records what must eventually become false: an
...
registration are both failures. `docs/provisional/README.md` is the format
contract.
...
mutable one would be the more visible of the pair
(`docs/policy/decisions/repository/annotated-tag-is-the-release-record.md`).
