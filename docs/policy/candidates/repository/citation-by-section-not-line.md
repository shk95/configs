...
- `docs/decisions/…` and `docs/candidates/…` path citations (now under `docs/policy/`), 28 unique, 0
...
- The `§` form the fix uses is not new here: `invariants/*` entries already
  spelled `decision: docs/decisions/<slug>.md § <Section>` (now
  `docs/policy/decisions/<scope>/<slug>.md`), and a candidate's
  own `target:` key uses it.

## Occurrences

- 2026-09-10: four broken citations corrected in place; the correction did
  not wait for this candidate, per `docs/policy/candidates/README.md` ("the
