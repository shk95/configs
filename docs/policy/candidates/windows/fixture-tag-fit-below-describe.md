...
target: windows/tests/WinEnv.Tests.ps1 (retag or untag the cases below); docs/policy/invariants/README.md § Fixture units name invariants (a negative case that proves the scanner is not tagged with the statement's id); tool/version-control/test (the 1,081 lines before its first banner hold 13 tags in no unit, a known limit in docs/policy/decisions/repository/fixture-tags-name-proven-invariants.md with no issue or owner)
...
- `tool/version-control/invariants` C5 and C10; `docs/policy/decisions/repository/fixture-tags-name-proven-invariants.md` ("Known limit").
