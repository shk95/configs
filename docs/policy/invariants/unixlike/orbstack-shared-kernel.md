...
more than was observed. Each machine runs in a user namespace of its own, not
the kernel's initial one: two machines read side by side on 2026-09-20 had
different ones, and it is also why the debug file system cannot be mounted
there. On a kernel this recent a `binfmt_misc` registry belongs to a user
namespace, so a registration made in a machine would probably stay in it.
That was not tried, because trying it means writing to a registry whose reach
is the question. The rule does not depend on the answer: the machine has no
use for a registration of its own, emulation there is OrbStack's, and it is
the care the repository already takes for WSL's distributions, where the
shared registry was broken once and traced for a day.
