...
enforced. `docs/policy/architecture.md`, "Invariant registry", records why the
...
rationale: <AGENTS.md | docs/policy/architecture.md> § <heading>
...
| `rationale` | 1 | Where layer 2 justifies it. Only `AGENTS.md` or `docs/policy/architecture.md`; the heading must exist. |
| `enforced-by` | 1+ | How it is kept. See kinds. |
| `owner` | 0–1 | Required for `manual` and `pending`. |
| `decision` | 0+ | A decision record the entry rests on, usually under `docs/policy/decisions/`; the heading must exist. |
...
| `manual` | the evidence item a reviewer produces, in words | the scope's `docs/policy/definition-of-done/<scope>.md` contains `INV <id>`; `owner` |
...
statement is the reviewer's to confirm; the definition of done says so.

## Example

`docs/policy/invariants/repository/registry-coverage.md` is the registry's own entry and
