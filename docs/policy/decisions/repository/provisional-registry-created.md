...
The repository enumerates what must remain true under `docs/policy/invariants/<scope>/`
...
As of 2026-09-05 such measures are registered under `provisional/<scope>/` (now `docs/provisional/<scope>/`),
...
- Recording such items only as open conditions in `docs/status/`. That is
