...
four of eleven commits changed only `docs/` or `invariants/` (now under `docs/`), and each paid
...
runs the real registry checker over `docs/` and `docs/policy/invariants/`, which is the
