...
one judgement point (`docs/policy/decisions/repository/commit-marks-a-judgement-point.md`).
...
lands (`docs/policy/architecture.md`, "Common domain"). The planned migration is the
...
(`docs/policy/decisions/repository/ci-evidence-without-hosted-runners.md`). The correct
...
It is not registered under `docs/provisional/`. That registry records what must
