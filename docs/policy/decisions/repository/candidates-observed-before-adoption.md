...
withdrawn, and a registry count in the status file that went stale with the
...
`docs/policy/candidates/`, one file per candidate, tracked so that every clone and
