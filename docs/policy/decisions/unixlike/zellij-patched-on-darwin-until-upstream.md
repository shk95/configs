...
(`docs/policy/decisions/unixlike/nixpkgs-overlays-declared-once.md`), and the measure is
registered as temporary in `docs/provisional/unixlike/zellij-combining-marks.md`,
...
`README.md` line, the `Justfile` recipe, the registry entry and all three
status mentions of the measure — the Unix-like paragraph and the tagged open
condition in `docs/status/unixlike.md`, and the provisional registry count in
`docs/status/repository.md` — and adds a dated
line to this record and to the `docs/reference/troubleshooting.md` entry. Only the open
