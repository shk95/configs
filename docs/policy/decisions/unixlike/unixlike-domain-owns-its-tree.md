...
them (`64a2a85:docs/candidates/domain-tool-placement-and-justfile-exception.md`, deleted when promoted,
...
(`docs/policy/decisions/windows/windows-entry-point-in-domain.md`). It includes
...
`64a2a85:docs/candidates/domain-tool-placement-and-justfile-exception.md` was
...
derivation; `docs/work/repository/unixlike-restructure-study/study.md` holds the spike, the
...
(`repository`, registered under `provisional/` (now `docs/provisional/`) with `PROV` tags because the
...
own removal (`docs/policy/architecture.md § Common domain`). Until the move,
`docs/policy/architecture.md § Unix-like domain` and `AGENTS.md § Goal and
