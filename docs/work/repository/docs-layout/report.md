...
| AC4 | pending | After step 1, outside `unixlike/`, `windows/` and `docs/work/`, the search finds only exempt hits: the tree drawing in `docs/README.md`, the decision record's account of the old layout, and the classifier's transitional arms and their fixtures. Three hits fall outside the exemptions as AC4 words them and are dated accounts that name the new path beside the old one: `docs/status/repository.md` ("`docs/design/` (since 2026-09-19 `docs/work/`)") and two lines of `docs/policy/candidates/repository/citation-by-section-not-line.md`; they are recorded here for the step 3 search to settle. The two citations of the deleted candidate are pinned to `64a2a85`. 22 files in the domain trees remain for steps 2 and 3. |
