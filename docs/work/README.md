...
`docs/policy/architecture.md`, the registries, the tools, the hooks or CI rests on
one, and an agent does not follow one as a rule.
`docs/policy/decisions/repository/design-documents-outside-the-authority-model.md` records why
...
evidence and a `docs/status/` line rather than a report; this area reads
...
like a policy document to carry, which waits in `docs/policy/candidates/`. Scratch
thinking that promises nothing stays untracked in `notes/`.

## Format

One directory per document, `docs/work/<scope>/<slug>/`, holding `<kind>.md`. Line 1 is `# <Title>`; then a
header of `key: value` lines up to the first blank line, parsed the way
decision records are (`docs/policy/decisions/README.md`); then prose.
...
state rather than a measurement has taken the status files' work and will be
wrong without anyone noticing.

There is no checker for this directory, for the reason `docs/policy/candidates/` has
