...
| AC5 | pending | Private `shk95/configs-hosts` PR #1 merged seven independent host flakes and locks, pinned to provider merge `66fed098`. All seven final `drvPath` values matched the `e8730ce` baseline; local `tool/check-hosts` and Linux CI run `35979483776` passed. Darwin built natively on aarch64 macOS, and the OrbStack consumer built natively in its already running aarch64 Linux machine. The other installed Linux and WSL consumers still need native builds; all consumers need their own runtime evidence. The physical desktop is uninstalled. No consumer activation was requested. |
| AC6 | pending | |
| AC7 | pending | |
| AC8 | pending | |
...
- Build: The existing Darwin output built natively on aarch64 macOS after the API refactor. CI run `35975584104` built the standalone x86_64 Linux home. The private consumer's Darwin output built natively on aarch64 macOS. Its OrbStack output built on the already running aarch64 Linux machine with `nix build --no-link --max-jobs 2`; the resulting system path differed from `/run/current-system`. Other installed Linux and WSL consumers still need matching builds.
