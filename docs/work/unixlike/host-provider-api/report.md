...
| AC5 | pending | Private `shk95/configs-hosts` PR #1 merged seven independent host flakes and locks, pinned to provider merge `66fed098`. All seven final `drvPath` values matched the `e8730ce` baseline; local `tool/check-hosts` and Linux CI run `35979483776` passed, and Darwin built natively on aarch64 macOS. Installed Linux and WSL consumers still need native build and runtime evidence; the physical desktop is uninstalled. No consumer activation was requested. |
| AC6 | pending | |
| AC7 | pending | |
| AC8 | pending | |
...
- Build: The existing Darwin output built natively on aarch64 macOS after the API refactor. CI run `35975584104` built the standalone x86_64 Linux home. The private consumer's Darwin output also built natively on aarch64 macOS; installed Linux and WSL consumers still need their own matching builds.
